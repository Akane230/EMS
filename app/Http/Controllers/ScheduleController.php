...
class ScheduleController extends Controller
{
    ...
    /**
     * Export individual schedule to PDF
     */
    public function exportIndividualPdf(Schedule $schedule)
    {
        $schedule->load(['section', 'instructor', 'room', 'course']);

        $pdf = PDF::loadView('schedules.individual-pdf', [
            'schedule' => $schedule,
            'title' => 'Schedule Record'
        ]);

        return $pdf->download('schedule_record_' . $schedule->id . '_' . now()->format('Y-m-d') . '.pdf');
    }
}
